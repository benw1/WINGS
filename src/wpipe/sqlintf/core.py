...
if sqlite:
    ...
    raise ImportError(
        "You must provide an engine URL via the environment variable WPIPE_ENGINEURL"
    )
            ...
            if Path(hostname).is_file():
                ...
                engine_url = url_parse_results._replace(
                    netloc=url_parse_results.netloc.replace(
                        f"@{url_parse_results.hostname}", f"@{ip}"
                    )
                ).geturl()
                hostname = ""
            else:
...
if not sqlite:
    ...
    url_parse_results = urllib.parse.urlparse(ENGINE_URL)
    ENGINE_URL = url_parse_results._replace(
        path=url_parse_results.path.replace("server", "wpipe")
    ).geturl()
    Engine.dispose()
    Engine = make_engine()
