...
def process_catalog(job_id,event_id):
   ...
   _t = subprocess.call(['cp',fileroot+'/'+filename,
                         myTarget.relativepath+'/proc/.',
                         stdout=subprocess.PIPE])
    
   fileroot = myTarget.relativepath+'/proc/'
   ...
   RA, DEC, M1, M2, M3, M4, M5 = \
      data['col1'], data['col2'], data['col3'], data['col4'],\ # column names should be filternames
   ...
   for j,filt in enumerate(filternames):
      ...
      galaxies = wtips([str(myPipe.softRoot)+
                        '/'+filt+'_galaxies.txt'])      # this file will be provided pre-made

      galaxies.flux_to_Sb()                             # galaxy flux to surface brightness

      if j==0:                                          # only get random positions once
         radec = galaxies.random_radec_for(stars)       # random RA DEC across star field
         Parameters.addParam(myConfig,'RA',
                             float(stars.center[0]))
         Parameters.addParam(myConfig,'DEC',
                             float(stars.center[1]))
      galaxies.replace_radec(radec)                     # distribute galaxies across starfield

      stars.merge_with(galaxies)                        # merge stars and galaxies list

      outfile = 'Mixed_'+outfile
...
if __name__ == '__main__':
   ...
   if args.REG:
      ...
      process_catalog(job_id,event_id)
