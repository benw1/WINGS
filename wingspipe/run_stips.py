...
def run_stips(event_id, dp_id, ra_dith, dec_dith):
    ...
    #need to check that the with statment below works as intended
    with fits.open(filename) as myfile:
        head = [next(myfile[1].data) for x in range(3)]
    #check that the .split works as intended with fits files
    pos = head[2].split(' ')
    crud,ra = pos[2].split('(')
...
if __name__ == '__main__':
    ...
    #line above gives key error - maybe check process_catalogues?
