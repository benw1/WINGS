...
def process_fixed_catalog(job_id,event_id,dp_id):
   ...
   try:
      ...
      for stips_cat in stips_files:
         ...
         for k in range(int(ra_dithers)):
            ...
            for j in range(int(dec_dithers)):
               ...
               dithfilepath = stipsfilepath.replace(''.join(['_',str(filtroot)]),''.join(['_',str(dithnum),'_',str(filtroot)]))
               ...
               _dp = DataProduct(filename=dithfilename,relativepath=myConfig.procpath,group='raw',configuration=myConfig).create()
               newdpid = int(_dp.dp_id) 
               event = Job.getEvent(myJob,'new_stips_catalog',options={'dp_id':newdpid,'to_run':total,'name':comp_name,'ra_dither':ra_dither,'dec_dither':dec_dither})
               dithnum += 1
               logprint(myConfig,myJob,''.join(["Firing event ",str(event['event_id'].item()),"  new_stips_catalog"]))
               fire(event)
         i += 1
      logprint(myConfig,myJob,"Dither Success")
      print("Dither Success process")

   except:
      #except "ksdf":
      logprint(myConfig,myJob,"No Dithers Found")
      ...
      for stips_cat in stips_files:
         ...
         _dp = DataProduct(filename=stips_cat,relativepath=myConfig.procpath,group='proc',filtername=filtname,subtype='stips_input_catalog',configuration=myConfig).create()
         dpid = int(_dp.dp_id)
         event = Job.getEvent(myJob,'new_stips_catalog',options={'dp_id':dpid,'to_run':total,'name':comp_name,'ra_dither':0.0,'dec_dither':0.0})
         logprint(myConfig,myJob,''.join(["Firing event ",str(event['event_id'].item()),"  new_stips_catalog"]))
         fire(event)
         i += 1
      ...
      try:
         ...
         for k in range(int(ra_dithers)):
            ...
            for j in range(int(dec_dithers)):
               ...
               event = Job.getEvent(myJob,'new_stips_catalog',options={'dp_id':dpid,'to_run':total,'name':comp_name,'ra_dither':ra_dither,'dec_dither':dec_dither})
               logprint(myConfig,myJob,''.join(["Firing event ",str(event['event_id'].item()),"  new_stips_catalog"]))
               fire(event)
