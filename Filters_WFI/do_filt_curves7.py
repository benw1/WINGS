...
filternames = np.array(['X625', 'Z087','Y106','J129','H158','F184','W149'])
filt_set    = np.array([[0.48,0.76],[0.76,0.977],[0.927,1.192],[1.131,1.454],[1.38,1.774],[1.683,2], [0.927,2]])
filt_set2   = np.array([[0.46,0.79],[0.75,1],    [0.91,1.23],  [1.1,1.5],    [1.35,1.85], [1.65,2.1],[0.9,2.05]])
...
def DoAll():    
    for filename,tablename in zip(filenames,tablenames):
        for i in [0]:
            tmp = [i,1,2,3,4,5,6]
            filtername = filternames[tmp]
            filtset = filt_set[tmp]
            filtset2 = filt_set2[tmp]
            make_filters(filename,tablename,filternames=filtername,filt_set=filtset,filt_set2=filtset2)
...
def make_filters(filename,tablename,filternames=filternames,filt_set=filt_set,filt_set2=filt_set2,
    ...
    plot_filter_set(tablename,A,wave,filternames,title=title,filt_set=filt_set,plotfilename=plotfilename)
    return
...
def write_ascii_table(tablename,A,wave,filternames=filternames):
    ...
    tab = [wave,A[0,],A[1,],A[2,],A[3,],A[4,],A[5,],A[6,]]
    nms = ('wavelength','X625','Z087','Y106','J129','H158','F184','W149')
    fmt = {'wavelength':'%15.10f','X625':'%8.4f','Z087':'%8.4f',\
