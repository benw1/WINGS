...
foreach $line1 (@files){
    ...
    $cmderr = $pipeline->getSwBuildDir() . "/pipe\_3.0/make_cmd.py";
    
    foreach ($i=0;$i<$filters1->length-1;$i++){
	foreach ($j=$i+1;$j<$filters1->length;$j++){    
	    $filt1 = @filters[$i];    
	    $filt2 = @filters[$j];
	    $outfile = $gal . "_" . $filt1 . "_". $filt2 . ".gst_cmd.png" if $line =~ /\.gst/;
	    $outfile2 = $gal . "_" . $filt1 . "_". $filt2 . ".gst_gridcmd.png" if $line =~ /\.gst/;
	    $outfile = $gal . "_" . $filt1 . "_". $filt2 . ".st_cmd.png" unless $line =~ /\.gst/;
	    $outfile2 = $gal . "_" . $filt1 . "_". $filt2 . ".st_gridcmd.png" unless $line =~ /\.gst/;
    ...
    system "eval `/home/puser/.ureka/ur_setup -sh \$*` $cmderr $line -bin $gridsize -x1 $col1 -y1 $mag2 -x2 $col2 -y2 $mag1 -lvl \[$clevels\] -thr $threshold -pid $pid -targ $targname -grid $gridbreak >> $logfile 2>&1\n";
    &logprint("eval `/home/puser/.ureka/ur_setup -sh \$*` $cmderr $line -bin $gridsize -x1 $col1 -y1 $mag2 -x2 $col2 -y2 $mag1 -lvl \[$clevels\] -thr $threshold -pid $pid -targ $targname -grid $gridbreak >> $logfile 2>&1\n");

    # system "eval `/home/puser/.ureka/ur_setup -sh \$*` $cmderr $line >> $logfile 2>&1\n";
    # &logprint("eval `/home/puser/.ureka/ur_setup -sh \$*` $cmderr $line >> $logfile 2>&1\n");
    
    unless (-e $outfile){
	die "NO COLOR MAGNITUDE DIAGRAM CREATED FOR $filt1 and $filt2 IN $gal\n";
    }
    &logprint("committing cmd files to s3...\n");
    my $dataproducts = $pipeline->selectBy($this_configuration, ['suffix','.png']);
