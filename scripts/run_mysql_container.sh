...
then
  docker run --detach --name=wingsmysql -v "${WINGS_DB_STORAGE}:/var/lib/mysql" --env="MYSQL_ROOT_PASSWORD=password" --publish 8000:3306 mysql:5.7.29
  sleep 2 # let container start
  mysql --host localhost -P 8000 --protocol=tcp -u root -p -e "create database server"
else
  docker run --detach --name=wingsmysql -v "${WINGS_DB_STORAGE}:/var/lib/mysql" --publish 8000:3306 mysql:5.7.29
fi